...
## 新增資源 (透過 BlockCost)
func add_cost(cost: BlockCost) -> void:
	var cost_set = ResourceSet.from_block_cost(cost)
	resources_set = resources_set.add(cost_set)

## 檢查是否能負擔指定的 BlockCost
func can_afford(cost: BlockCost) -> bool:
	var cost_set = ResourceSet.from_block_cost(cost)
	return resources_set.has_enough(cost_set)

## 扣除資源。若資源不足則回傳 false 並放棄扣除
func consume_cost(cost: BlockCost) -> bool:
	var cost_set = ResourceSet.from_block_cost(cost)
...
## 依據比例退還資源（用於拆除退款）
func refund_cost(cost: BlockCost, ratio: float = 1.0) -> void:
	var cost_set = ResourceSet.from_block_cost(cost)
